...
// Helper function to create a bloom beast card
...
// Helper function to create a magic card
function createMagicCard(id: string, name: string, cost: number): MagicCard {
  return {
    id,
    name,
    type: CardType.Magic,
...
// Helper function to create a trap card
function createTrapCard(id: string, name: string, cost: number, trigger: TrapTrigger): TrapCard {
  return {
    id,
    name,
    type: CardType.Trap,
...
describe('BloomBeasts Gameplay', () => {
  ...
  describe('Game Initialization', () => {
    test('should initialize a new game with two players', () => {
      ...
      expect(state.gameData.players[0].currentHealth).toBe(30);
      expect(state.gameData.players[1].currentHealth).toBe(30);
  ...
  describe('Card Playing', () => {
    ...
    test('should not play bloom to occupied position', () => {
      ...
      expect(result.success).toBe(false);
      expect(result.error?.message).toContain('Invalid card placement');
    });
  });
  ...
  describe('Combat', () => {
    ...
    test('should attack player directly when no blocker', () => {
      ...
      expect(newState.gameData.players[1].currentHealth).toBe(27);
    });
  ...
  describe('Win Conditions', () => {
    ...
    test('should end game when player health reaches 0', () => {
      ...
      expect(endState.gameData.players[1].currentHealth).toBeLessThanOrEqual(0);
  ...
  describe('Complex Scenarios', () => {
    ...
    test('should validate action order in a turn', () => {
        ...
        players: [
          {
            ...
            type: Turbo.PlayerType.HUMAN,
            metadata: {
              deck: [createBloomBeast('beast1', 'Test', 1, 1, 1)]
            }
          },
          { id: 'player2', name: 'Bob', type: Turbo.PlayerType.HUMAN, metadata: { deck: [] } }
        ]
      ...
      expect(secondDraw.error?.message).toContain('Already drawn a card');
    });
  });
});