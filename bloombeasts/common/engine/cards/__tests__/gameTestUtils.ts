 ...
 * Note: This file provides utilities for testing card DATA and structure,
 * not for testing the full game engine which has been replaced by the TURBO system.
 */
...
/**
 * Create a deck list for testing
 * @deprecated Use testUtils.ts helpers for testing card data directly
 */
export function createTestGame(): never {
  throw new Error(
    'createTestGame() is no longer available. The GameEngine system has been replaced with TURBO. ' +
    'Use testUtils.ts validators to test card data, or use BattleController for integration tests.'
  );
}
