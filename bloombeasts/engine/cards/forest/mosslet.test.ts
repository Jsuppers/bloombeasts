...
describe('Mosslet Card', () => {
  ...
  describe('Base Ability - Spores of Defense', () => {
    test('should have valid structured ability', () => {
      validateStructuredAbility(MOSSLET.ability);
    });

    test('should have correct ability name', () => {
      expect(MOSSLET.ability.name).toBe('Spores of Defense');
    });

    test('should trigger on damage', () => {
      expect(MOSSLET.ability.trigger).toBe(AbilityTrigger.OnDamage);
    });

    test('should apply 1 Spore counter to self', () => {
      expect(MOSSLET.ability.effects).toHaveLength(1);
      const effect = MOSSLET.ability.effects[0] as ApplyCounterEffect;
  ...
  describe('Card Consistency', () => {
    ...
    test('should have consistent spore mechanics theme', () => {
      const baseEffect = MOSSLET.ability.effects[0] as ApplyCounterEffect;
