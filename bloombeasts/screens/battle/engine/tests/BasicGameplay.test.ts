...
describe('BloomBeasts Basic Gameplay', () => {
  ...
  describe('Combat', () => {
    test('should attack enemy beast', () => {
      ...
      expect(defender.health).toBe(-1); // Should be destroyed
      expect(attacker.health).toBe(2); // Should survive
    ...
    test('should damage player directly', () => {
      ...
      state.players[1].health -= attacker.attack;

      expect(state.players[1].health).toBe(25);
    });
  ...
  describe('Win Conditions', () => {
    test('should win when opponent health reaches 0', () => {
      ...
      const isGameOver = state.players.some(p => p.health <= 0);
      const winner = state.players.find(p => p.health > 0);
    ...
    test('should handle draw when both players die', () => {
      ...
      const deadPlayers = state.players.filter(p => p.health <= 0);
